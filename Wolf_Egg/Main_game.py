...
class Wolf(arcade.Sprite):  # we create a class that will be responsible for the wolf's work
...
class Egg(arcade.Sprite): # we create a class that is responsible for creating eggs
    def __init__(self):  # create an initialization method to create parameters for all eggs
        super().__init__("egg.png", 1.1)  # load the initial texture for the egg, set its size to 110%
    ...
    def setup(self):  # create a method to set initial values
        self.pos = random.randint(1,4)  # randomly select one of four positions
